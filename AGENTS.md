...
- The existing data-only loot integration addon is generated by `/home/evand/Projects/minecraft/ce_pack_tools/Build-LootAddon.ps1`. Its source includes `Build-CEDatapacks.ps1`, `Add-AccessoryLoot.ps1`, and `Test-AccessoryLoot.py`. Build from a hash-verified inventory resolved from this repository, never from an unverified live instance. Preserve the shared Relics/Artifacts roll and Fargo's existing coverage; do not duplicate either.
- Launch Prism and Minecraft on **workspace 2**, using `agent-launch --workspace 2 -- <command> [args...]`.
- Artifacts is pinned to **13.2.3** because Reliquified Artifacts 1.0.8 requires that exact version. Recheck the compatibility addon's declared version range before changing either reference.
